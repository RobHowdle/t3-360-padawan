...
/*
  v1.3
  Further Edit by Rob Howdle for T3-M4 Adaption 2019. The sketch can now be adapted for use with  T3 Utility Droid. Permission has been given by Dan K to do this and a lot of the thanks for this go out to him for his assistancce.
  Adaptions are as follows:
  - Adding the ability to control T3's head in the up, down, 360 degree spin as well as left and right tilt.
  - Added 8 more sound combos spreading across A,B,X,Y and triggers to allow for more sounds to be played.
  ...
  v1.2
  Edited for "left hand drive" by STeve Baudains 2017.  Means movement controls on left stick
  move like video game and leaving dome and sound controls on right hand side of controller
  v1.1
  by Robert Corvus
...
char turnThrottle = 0;
int servoPin = 3;
// Servo on and off integer for spray
Servo myservo; //define servoname
int servoon;
int servooff;
...
void loop() {
  ...
  if (Xbox.getButtonPress(L1, 0)) {
    if (Xbox.getButtonPress(UP, 0)) {
      myservo.write(180); // was digitalWrite(EXTINGUISHERPIN, LOW)
      delay(100);
      myservo.write(0);
      // } else {
      // myservo.write(0); // was digitalWrite(EXTINGUISHERPIN, HIGH)
    }
  }
  ...
  // turn hp light on & off with Right Analog Stick Press (R3)
  ...
  rightStickValue = (map(Xbox.getAnalogHat(LeftHatY, 0), -32768, 32767, -drivespeed, drivespeed));
